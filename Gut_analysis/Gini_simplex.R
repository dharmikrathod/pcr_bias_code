...
gini_index <- function(pi_mat) {
  D <- ncol(pi_mat)
  gini_vec <- apply(pi_mat, 1, function(p) {
    sum_outer <- sum(outer(p, p, function(a, b) abs(a - b)))
    return(sum_outer / (2 * D))
  })
  return(gini_vec)
}


simpsons_index <- function(pi) {
  ## pi[pi <= 0] <- 1e-10
  ## pi <- as.matrix(pi)
...
gini.x <- gini_index(x.prop)
gini.y <- gini_index(y.prop)
delta_gini <- gini.y - gini.x
