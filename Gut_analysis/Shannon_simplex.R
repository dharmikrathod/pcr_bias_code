...
shannon.x <- shannon_index(x.prop)
shannon.y <- shannon_index(y.prop)
delta_shannon <- shannon.y - shannon.x
...
bias.plot <- t(data.frame(as.numeric(ilrInv(bias))) ) # Create the data frame
colnames(bias.plot) <- c('v1', 'v2', 'v3')
zero.plot <- data.frame(t(as.numeric(ilrInv(c(0, 0)))))  # Create transposed data frame
colnames(zero.plot) <- c('v1','v2', 'v3')  # Assign column names
